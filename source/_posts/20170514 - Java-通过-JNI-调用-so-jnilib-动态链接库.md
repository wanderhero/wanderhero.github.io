...
***
### 前言
&emsp;&emsp;其实 Java 方面不是大问题，关键问题在于如何生成 动态链接库。首先还是先了解下什么是 JNI，才能知己知彼，百战不殆。  
&emsp;&emsp;参考自 [**xyang0917**](#参考1)，JNI 全称是 Java Native Interface (Java本地接口) 单词首字母的缩写，本地接口就是指用 C 和 C++ 开发的接口。由于 JNI 是 JVM 规范中的一部份，因此可以将我们写的 JNI 程序在任何实现了 JNI 规范的 Java 虚拟机中运行。同时，这个特性使我们可以复用以前用 C/C++ 写的大量代码。  
...
- **首先获取 .class 字节码文件，如果没有生成，可以通过 javac 命令将 .java 源文件编译而成。**  
...
```
&emsp;&emsp;&emsp;参数说明，注意：-d 和 -o 只能使用其中一个参数，具体查看 **javah -help**：  
...
本文用的是 阿里云 的 Ubuntu 14.04.2 LTS，把 .h 和 .c 文件放到同一目录下，通过 gcc 命令一步生成 libJNITest.so。
...
```
&emsp;&emsp;&emsp;看上去也很简单，但是在我的电脑上却报错了，提示 **'jni.h' file not found(找不到 jni 头文件)**，感谢 [**Stack Overflow**](#参考2)，找到了貌似正确的命令，却提示 **Operation not permitted(不允许操作)**，加了 sudo 都不行，难道我的人品差到连系统都不忍直视了吗？事实证明，人品是挺差的，但功夫不负有心人，感谢 [**小胡子哥**](#参考3)，原来 Apple 在 OS X 10.11 以后的版本中默认启动了一项系统保护程序，叫做 System Integrity Protection，也被唤作 rootless（寓意让 root 弱一点），该程序意在保护电脑不被恶意程序攻击，感觉屌屌的，以安全的名义，一切都是可以接受的。  
